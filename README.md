...
Additionally, we provide, in the main folder, the notebook `revised_akpr_model_standalone.ipynb`, which is a stand-alone notebook implementing the revised AKPR model for the TCR/TCR and TCR/CAR antagonism. It contains all functions to evaluate the steady-state solutions (the same that are available in the `models` module). Parameter values are hardcoded in the notebook, such that it does not depend on data files either. 
...
Except for the stand-alone notebook, datasets and MCMC results should be downloaded (or generated locally) prior to running these notebooks, since the best parameter fits, the ligand numbers and antigen affinities are loaded in them. 
...
The order matters, as TCR/TCR model fits need to be generated and analyzed before TCR/CAR simulations are launched. These simulations should take in total 12-24h to run on a desktop computer or small computational cluster with multiple CPU cores. 
...
The analysis for antagonism and enhancement predictions in peptide mutant libraries, presented in Figure 5 and explained in Supplemental Information section II.3.9, is performed in two main notebooks for the two libraries:

- `universal_antagonism_mskcc_library.ipynb`: for the 7 peptide mutant libraries (3 original peptides, 7 different TCR from patient donors) provided in a recent paper (Luksza *et al*., *Nature*, 2022) from researchers at the Memorial Sloan Kettering Cancer Center (MSKCC), but using our own EC50 fits on their dose response measurements. 
...
Moreover, the scripts `secondary_scripts/mskcc_ec50_mcmc.py` and `secondary_scripts/hhatv4_ec50_mcmc.py` perform our estimation of peptide EC50s (with uncertainties) by fitting dose response curves with MCMC simulations. We provide the fit results (statistics of the MCMC distributions) in the [data and results Zenodo repository](#zenodo). Hence, running these scripts is optional -- note that since there are ~1400 dose response curves to fit, the whole fitting process takes $\sim$ 24 hours on a multi-CPU cluster and generates $\sim 10$ GBs of MCMC samples. 

See also `secondary_scripts/universal_antagonism_basic_cell_lines.ipynb` for a similar application, in other cell lines, of the model as a pipeline taking peptide EC50s as an input and returning predicted TCR/CAR fold-changes as an output. 
...
The Jupyter notebook `secondary_scripts/tcr_car_invivo_predictions.ipynb` shows that the model predictions of TCR/CAR antagonism, fitted on *in vitro* data, correlate relatively well with mouse *in vivo* survival data for tumors with OT-1 antigens (Figure 4, file `data/invivo/B16-combined_tumorMeasurements-unblinded.h5`). Performing a linear regression of the model output versus survival in single antigen tumors (TCR or CAR only), we attempted to predict survival in dual antigen tumors (TCR and CAR). There are, however, outliers, and the poor quantitative agreement does not support confident extrapolation of the model to *in vivo* or clinical settings. 
...
We acknowledge the full list of authors (see above) who contributed to the project. 
