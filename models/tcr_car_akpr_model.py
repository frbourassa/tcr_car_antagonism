...
def jak(r, i2, ec50lo, ec50hi, r0, base):
    ...
    coef = (b + mm_r) / (1. + b)
    return coef*i2 / (i2 + ec50hi*mm_r + ec50lo*(1. - mm_r))
...
### TODO: update this part with better description of
# cell numbers and Ag numbers over time.
def deriv_tcr_car_il2model(t, y, p):
    ...
    y = [Ag_TCR, Ag_CAR, IL-2R, IL-2]
    """
    d = np.zeros(y.shape)
    # Ag dynamics
    d[0] = -p["delta_a"]*p["N"]*p["Nstar"]/(p["N"] + p["Nstar"])*y[0]  # Ag
    d[1] = -p["delta_a"]*p["N"]*p["Nstar"]/(p["N"] + p["Nstar"])*y[1]  # Ag
    # TODO: is there internalization of CD19? Assuming yes and identical
    # to TCR for now, but could be a source of difference!

    # Compute C_N and D_N' with current Ag
    ## TODO: modify this part to incorporate a CAR receptor.
    # Number of ligands per APC for SHP-1 model is y[0]
    complexes = steady_akpr_i_receptor_types(
        ratesp=p.get("all_rates"),
        tausp=[p.get("tcr_tau"), p.get("car_tau")],
        lsp=y[0:2],
        Rsp=[p.get("tcr_Rtot"), p.get("car_Rtot")],
        ITp=p.get("I_tot"),
        nparams=p.get("n_params")
    )

    # Normalize C_N and D_N'
    joint_zap = np.sum(compute_zoutput_from_complexes(complexes, p.get("zap_thresholds")))

    # Compute other quasi-static variables
    jk = jak(y[2], y[3], p["ec50lo"], p["ec50hi"], p["r0"], p["stat_baseline"])
    # Activation of IL-2 downstream of TCR's and CAR's ZAP-70
    pon_cdn_i2 = joint_zap**p["n_ci2"]/(joint_zap**p["n_ci2"] + p["K_cni2"]**p["n_ci2"])
    # Activation of IL-2R downstream of TCR's and CAR's ZAP-70
    pon_cdn_r = joint_zap**p["n_cr"]/(joint_zap**p["n_cr"] + p["K_cnr"]**p["n_cr"])  # Activation of IL-2R by TCR
    ...
    if p["tcr_repress_stat"]:
        pon_j_s = pon_j_s * (1.0 - pon_cdn_i2)

    # IL-2R
    mrna_terms = p["rho_r0"] + p["rho_rc"]*pon_cdn_r + p["rho_rs"]*pon_j_s
    d[2] = p["k_r"]/p["delta_mr"]*mrna_terms - p["delta_r"]*y[2]

    # IL-2 (in pM)
    ## TODO: check if we can remove the bounded exponential. Probably, yes.
    ## TODO: maybe make nb T cells a dynamical variable instead of parameter.
    ## Would then need to include PI3K and cyclinD.
    d[3] = (1. - np.exp(-p["delta_mi2"]*t))*p["ksec_i2"]*p["N"]*pon_cdn_i2
    #d[3] = p["ksec_i2"]*p["N"]*pon_cdn_i2
    d[3] = d[3] - p["kon_i2"]*p["N"]*y[2]*y[3]  # consumption by IL-2R
    # Decay: helps to get 0 for non-agonists if n_ci2 > 1
    # d[3] = d[3] - p["delta_i2"]*y[3]
    return d
